...
def _sha_obj(obj: object) -> str:
    """Digest an arbitrary object by sorted JSON.

    Kept local rather than core.sha_mapping, which takes a mapping: this is also
    called on lists of allowed tools and MCP names. Same canonicalisation.
    """
...
# state helpers
# --------------------------------------------------------------------------- #
#: Atomic via the core, so a crash mid-write cannot leave a truncated baseline
#: that reads as corrupt on every future session.
